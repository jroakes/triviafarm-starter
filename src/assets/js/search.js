...
class SearchPosts {
  ...
  render(query) {
    ...
    slicedPosts.forEach((post) => {
      const { url, title, image } = post;

      wrapperEl.innerHTML += `
      <div class="w-full sm:w-1/2 md:w-1/3 self-stretch p-2 mb-2">
      	<a href="${url}">
      		<div class="grid grid-flow-col overflow-hidden h-auto border shadow shadow-md">
      			<img class="block h-24 w-24 flex-none object-cover" src="${image}" alt="${title}">
      			<div class="bg-white p-4 fill">
